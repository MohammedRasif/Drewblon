...
function DashboardTalks() {
  ...
  // Extract talks from results
  const talks = recordResponse?.results || [];

  const handleTabChange = (tab) => setActiveTab(tab);
  const handleCategoryChange = (category) => setSelectedCategory(category);

  const buttonVariants = {
    active: { backgroundColor: "#2563eb", color: "#ffffff", scale: 1.05 },
    inactive: { backgroundColor: "#e5e7eb", color: "#000000", scale: 1 },
  };
